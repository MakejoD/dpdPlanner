...
async function createCompleteExampleData() {
  try {
    ...
    const users = await Promise.all([
      ...
      prisma.user.upsert({
        where: { email: 'admin@poa.gov.do' },
        update: {},
        create: {
          email: 'admin@poa.gov.do',
          passwordHash: hashedPassword,
    ...
    // Primero eliminar objetivos existentes para evitar conflictos
    ...
    // Primero eliminar productos existentes para evitar conflictos
    ...
    const assignments = await Promise.all([
      ...
      prisma.activityAssignment.create({
        data: {
          ...
          userId: users.find(u => u.email === 'admin@poa.gov.do').id,
          isMain: true
        }
      }),
