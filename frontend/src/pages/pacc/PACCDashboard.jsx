...
import api, { paccAPI } from '../../utils/api';
...
const PACCDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [schedules, alerts, compliance, processes] = await Promise.all([
        paccAPI.schedules.getAll(),
        paccAPI.alerts.getAll(),
        paccAPI.compliance.getLatest(),
  ...
  const generateReport = async () => {
    try {
      ...
      const response = await paccAPI.reports.generateExecutive();
      
      // En el entorno real esto devolvería un blob o URL de descarga
